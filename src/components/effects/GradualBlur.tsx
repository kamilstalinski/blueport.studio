...
type DimensionKey = 'height' | 'width';
const responsiveKeys = (key: string): { mobile: keyof GradualBlurProps; tablet: keyof GradualBlurProps; desktop: keyof GradualBlurProps } => {
  const k = key.charAt(0).toUpperCase() + key.slice(1) as 'Height' | 'Width';
  return { mobile: `mobile${k}` as keyof GradualBlurProps, tablet: `tablet${k}` as keyof GradualBlurProps, desktop: `desktop${k}` as keyof GradualBlurProps };
};
...
const useResponsiveDimension = (
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      const { mobile, tablet, desktop } = responsiveKeys(key);
      let v: string | undefined = config[key] as string | undefined;
      if (w <= 480 && config[mobile]) v = config[mobile] as string;
      else if (w <= 768 && config[tablet]) v = config[tablet] as string;
      else if (w <= 1024 && config[desktop]) v = config[desktop] as string;
      setVal(v);
    };
...
const GradualBlur: React.FC<GradualBlurProps> = props => {
  ...
  const containerStyle: CSSProperties = useMemo(() => {
    ...
    const baseStyle: CSSProperties = {
      ...
      pointerEvents: config.hoverIntensity && !isDisabled ? 'auto' : 'none',
      opacity: isDisabled ? 0 : isVisible ? 1 : 0,
      transition: config.animated
        ? `opacity ${config.duration} ${config.easing}`
        : isPageTarget
          ? 'opacity 0.3s ease-out'
          : undefined,
      zIndex: isPageTarget ? (config.zIndex >= 0 ? config.zIndex + 100 : Math.abs(config.zIndex)) : config.zIndex,
      ...config.style
    };
    ...
    return baseStyle;
  }, [config, responsiveHeight, responsiveWidth, isVisible, props.disabled]);
